...
**Validated (E prep, 2026-06-22):** against the live test server — dynamic client registration with our public redirect URI returned **201** (redirect allowlist not a blocker for that box); with `OD_PUBLIC_BASE_URL` set, `POST /api/mcp/oauth/start` produced a complete authorize URL with a **public callback**, the daemon's own registered `client_id`, **PKCE S256**, scopes `mcp:read mcp:write offline_access`, and RFC 8707 `resource`. **E full — COMPLETE (2026-06-22):** user approved consent in-browser → callback landed through the public proxy → token persisted to `mcp-tokens.json` (`connected:true`, access+refresh+tokenEndpoint+clientId stored) → an authenticated `initialize` + `tools/list` against the server returned **HTTP 200 with 106 tools** (the user's own connected Gmail/Apollo/etc. via the MCP gateway). **The complete per-user MCP OAuth/PKCE loop is validated end-to-end on this codebase** — no Open Design code change required.
