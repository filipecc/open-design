...
| **work-item** | `priority` (critical/high/medium/low), `completed`, `blocked-by`, `specs`, `blocked-by`, `specs` | backlog → assigned → in-progress → **done** · **archived** |
...
**Claude should:** set `in-progress` when starting work. Do **not** set `done` — that requires human review approval.
